...
#include <iostream>
#include "hyperv_switch.h"


void
hyperv_switch::activate(bool init, bool special) {
    if (init) {
...
void
hyperv_switch::deactivate() {

}


void
hyperv_switch::install(uint16_t dst_mac,
                       uint16_t port) {
    pi_p4info_t * p4info = controller->get_p4info();
    {
        ...
        header.set_value(ETH_DST_BYTE_OFFSET, dst_mac);
